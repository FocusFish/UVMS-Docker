...
psql -U asset -d $1 --host=$2 --single-transaction -q -f fish.focus.uvms.asset.liquibase-${unionvms.project.asset.module}.sql >/dev/null
echo "Running  audit.sql to create tables and init data"
psql -U audit -d $1 --host=$2 --single-transaction -q -f fish.focus.uvms.audit.liquibase-${unionvms.project.audit.module}.sql >/dev/null
echo "Running config.sql to create tables and init data"
psql -U config -d $1 --host=$2 --single-transaction -q -f fish.focus.uvms.config.liquibase-${unionvms.project.config.module}.sql >/dev/null
echo "Running exchange.sql to create tables and init data"
psql -U exchange -d $1 --host=$2 --single-transaction -q -f fish.focus.uvms.exchange.liquibase-${unionvms.project.exchange.module}.sql >/dev/null
echo "Running movement.sql to create tables and init data"
psql -U movement -d $1 --host=$2 --single-transaction -q -f fish.focus.uvms.movement.liquibase-${unionvms.project.movement.module}.sql >/dev/null
echo "Running movementrules.sql to create tables and init data"
psql -U movementrules -d $1 --host=$2 -a --single-transaction -q -f fish.focus.uvms.movement-rules.liquibase-${unionvms.project.movementrules.module}.sql >/dev/null
echo "Running incident.sql to create tables and init data"
psql -U incident -d $1 --host=$2 -a --single-transaction -q -f fish.focus.uvms.incident.liquibase-${unionvms.project.incident.module}.sql >/dev/null
echo "Running usm.sql to create tables and init data"
psql -U usm -d $1 --host=$2 --single-transaction -q -f fish.focus.uvms.user.liquibase-${unionvms.project.user.module}.sql >/dev/null
